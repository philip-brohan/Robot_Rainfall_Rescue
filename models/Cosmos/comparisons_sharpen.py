...
def test_image_size_effects():
    ...
    sharpen_counts = [50, 100, 150]

    for sharpen in sharpen_counts:
        resized_image = original_image.copy()
        if sharpen != 0:
            resized_image = resized_image.filter(
                ImageFilter.UnsharpMask(radius=2, percent=sharpen, threshold=3)
            )
        image_tensor = image_to_tensor(resized_image)
        reconstructed_image = autoencode(image_tensor, encoder, decoder)
        plot_images(
            resized_image, reconstructed_image, file_name=f"outputs/test_{sharpen}.png"
        )
