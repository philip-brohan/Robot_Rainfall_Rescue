...
weights_dir = (
    "%s/ML_ten_year_rainfall/models/ATB2_retuned/" + "Epoch_%04d"
...
def plotBox(ax):
    ...
    for yrl in range(1, 10):
        x = (
            imp["monthsWidth"]
            + yrl * (1.0 - imp["meansWidth"] - imp["monthsWidth"]) / 10
        )
        tp = topAt(x)
        bm = bottomAt(x)
        ax.add_line(
...
# Plot target using same method as original plot
ax_target = fig.add_axes([0.40, 0.23, 0.62, 0.97])
ax_target.set_xlim([0, 1])
ax_target.set_ylim([0, 1])
ax_target.set_axis_off()

# Plot years
def plotYearsTarget(ax):
...
plotBox(ax_target)
plotFixedText(ax_target)
plotYearsTarget(ax_target)
plotRainfallTarget(ax_target)

# Plot encoded using same method as original plot
ax_encoded = fig.add_axes([0.40, -0.27, 0.62, 0.97])
    ...
    for ydx in range(10):
        ...
        for dgi in range(4):
            ...
            if bestTranscribed == originalDigit:
                colour = "blue"
            if bestTranscribed == 10:
                btps = "_"
            else:
                btps = "%1d" % bestTranscribed
            ax.text(
                tp[0] - 0.02 + dgi * 0.015,
                lft[1],
                btps,
...
# Plot the transcribed numbers
plotBox(ax_encoded)
plotFixedText(ax_encoded)
plotYearsEncoded(ax_encoded)
plotRainfallEncoded(ax_encoded)
