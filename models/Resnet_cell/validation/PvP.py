...
from transcriberModel import resnetRRR
...
transcriber = resnetRRR()
weights_dir = ("%s/Robot_Rainfall_Rescue/models/resnetCell/" + "Epoch_%04d") % (
    os.getenv("SCRATCH"),
    args.epoch - 1,
)
...
testImages = getImageDataset(purpose="test", subdir="perturbed", nImages=args.nimages)
testNumbers = getNumbersDataset(
    purpose="test", subdir="perturbed", nImages=args.nimages
)
