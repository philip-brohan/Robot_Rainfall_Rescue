# Model specification for a resnet-based image analyser
...
def resIdentity(x, filters, reg=tf.keras.regularizers.l2(0.10), dropout=0.0):
...
def resStride(x, s, filters, reg=tf.keras.regularizers.l2(0.10), dropout=0.0):
...
#    x = tf.keras.layers.MaxPooling2D((3, 3), strides=(2, 2))(x)
    ...
    # Map input to a feature vector with 1x1 convolution
    #x = tf.keras.layers.Conv2D(
    #    44,
    #    kernel_size=(1, 1),
    #    strides=(1, 1),
    ...
    # Single fully connected layer to make the output
    x = tf.keras.layers.Flatten()(x)
    x = tf.keras.layers.Dense(44)(x)

    # Define the model
    model = tf.keras.Model(inputs=inputs, outputs=x, name="ResnetCNR")
    return model
