...
# Compare one of the test images with original and reconstucted corners overlaid
...
ax_original.set_xlim([0, 64])
ax_original.set_ylim([102, 0])
ax_original.set_aspect("auto")
ax_original.imshow(
    tf.reshape(originalImage, [102, 64]),
    cmap="gray",
    vmin=0,
    vmax=1,
    aspect="auto",
...
for point in range(22):
    ax_original.add_patch(
        matplotlib.patches.Circle(
            (cset[point * 2] * 64, (1 - cset[point * 2 + 1]) * 102),
            radius=1,
            facecolor="red",
            edgecolor="red",
            linewidth=0.1,
