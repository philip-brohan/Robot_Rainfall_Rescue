...
# Functions to make training and extraction datasets.
import os
...
from daily_rainfall.structured_output.generic.debug import pretty_batch_summary
...
def load_model_from_save(model_id, base_model_id, model_class, processor_class, device):
    # If only base model, load that (this will be the case for extraction runs where we haven't fine-tuned a model, but just want to run inference with the base model)
    if model_id is None:
        model_kwargs = dict(
            ...
            trust_remote_code=True,  # Required by Phi-4-multimodal-instruct and some other models with custom code on Hugging Face
        )
        model = model_class.from_pretrained(base_model_id, **model_kwargs)
        proc = processor_class.from_pretrained(base_model_id)
        return model, proc
    else:
        model_dir = f"{os.getenv('PDIR')}/{model_id}"
    ...
    try:
        ...
        model = model_class.from_pretrained(model_dir, dtype=dtype)
        model.to(device)
        return model, proc
    except Exception:
